...
const Main = () => {
    ...
    useEffect(() => {
        ...
        const process = async () => {
            ...
            for (let i = 39; i <= 39 /*100*/; i += 2) {
                try { queryImages.push(await utils.loadImage('../testing/videos/p1/out' + String(i).padStart(3, '0') + '.png')); } catch (e) { }
            }
            ...
            const controller = new MINDAR.Controller({
                ...
                frameDetection: { top: 0.25, right: 0.25, bottom: 0.25, left: 0.25 }
            });
            ...
            const { dimensions, matchingDataList, trackingDataList } = await controller.addImageTargets('../examples/image-tracking/assets/band-example/raccoon.mind');
