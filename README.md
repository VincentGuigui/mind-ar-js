...
- `node testing/white-border-realimage-signature.test.mjs` — **real photo + content signature**:
  computes the reference signature the authoring way (`MINDAR.WHITE_BORDER.computeSignatureFromImage`)
  from an INDEPENDENT flat photo of the card (`white_frame_postcard_flat.jpg`, detected + rectified),
  then matches the separate perspective photo against it — asserts the correct card is accepted and
  oriented, and a colour-inverted (wrong) signature is rejected; writes the tracking overlay +
  rectified flat reference to `testing/.tmp/`.
